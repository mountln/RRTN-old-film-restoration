...
def degradation_video_list_4(video_list,texture_url='./noise_data'): ## Add moving lines
    ...
    for x in video_list:
        ...
        frame_pil = transfer_1(x)
        frame_cv2 = transfer_2(frame_pil.convert("RGB"))
        gt_L.append(add_sharpening(frame_cv2))
...
def degradation_video_list_4_one_channel(video_list,texture_url='./noise_data'): ## Add moving lines
    ...
    for x in video_list:
        ...
        degraded.append(np.expand_dims(np.clip(np.array(color_jitter(x2)) / 255., 0, 1), -1))
